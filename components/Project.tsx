...
const Project: React.FC<ProjectProps> = ({ title, description, repoLink, liveLink,image }) => {
  return (
    <div className="relative group bg-white p-6 rounded-lg shadow-lg h-fit  max-w-72 min-h-92 flex flex-col justify-between items-center hover:drop-shadow-xl transition ease-in-out duration-300">
      <div className=''>
        <h3 className="text-xl font-bold mb-2">{title}</h3>
        <p className="text-gray-700 mb-4">{description}</p>
        
        <Image src={`/${image}`} alt="photo1" width={220} height={260} className='shadow-lg rounded-sm h-40'/>
      </div>
